...
func (productController *ProductController) GetAllProducts(c echo.Context) error {
	...
	productsWithStore := productController.productService.GetAllProductsByStore(store)
...
func (productController *ProductController) UpdatePrice(c echo.Context) error {
	...
	if len(newPrice) == 0 {
		return c.JSON(http.StatusBadRequest, response.ErrorResponse{
			ErrorDescription: "Parameter new price is required !",
		})
	}
	convertedPrice, err := strconv.ParseFloat(newPrice, 32)
	if err != nil {
		return c.JSON(http.StatusBadRequest, response.ErrorResponse{
			ErrorDescription: "New price is not a valid float !",
		})
	}
